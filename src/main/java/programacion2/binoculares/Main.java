...
import programacion2.binoculares.MainWindow;
...
public class Main {    
  public static void main(String[] args) {
      ...
      Simulador simulador = new Simulador(new Binocular());
      w.setVisible(true);
      
      String dir = w.getDireccion();
      while (true){
          if (w.getDireccion().equals(dir)){
          }else{
            dir = w.getDireccion();
            simulador.setDireccion(dir);
            
            Objetivo obj = simulador.observar();
            
            w.setDialogText(obj.getNombre());
            
            ArrayList<String> diario_str = new ArrayList<String>();
            for (int x = 0; x < simulador.getDiario().size(); x++) {
                String elem = simulador.getDiario().get(x).getNombre();
                diario_str.add(elem);
            }
            w.updateDiario(diario_str);
          }
          try {
              Thread.sleep(600);
